    ...
    namespace mdt
    {
        ...
        class MixedDataType
        {
          public:
            ...
            void add(const std::string &key, const int8_t &value)
            {
                ...
                d.data.emplace_back((uint8_t) value);
                data_.emplace_back(d);
            }
            void add(const std::string &key, const uint8_t &value)
            {
                int ch = findIndex(key);
                if (ch != -1)
                {
            ...
            void add(const std::string &key, const std::string &value)
            {
                ...
                if (ch != -1)
                {
                    if (data_.at(ch).data_type == DATA_TYPE_STRING)
                    {
                        ...
                        for (auto c : value)
                            data_.at(ch).data.emplace_back((uint8_t) c);
                        return;
                    } else
                        data_.erase(data_.begin() + ch);
                }
                ...
                for (auto c : value)
                    d.data.emplace_back((uint8_t) c);
                data_.emplace_back(d);
            }
            kDataTypes getType(const std::string key)
            {
                int i = findIndex(key);
                if (i != -1)
                {
                    return (kDataTypes) data_.at(i).data_type;
                }
          ...
          protected:
            ...
            int serialize(std::vector<uint8_t> &buffer)
            {
                ...
                {
                    buffer.emplace_back((uint8_t) d.data_type);
                    bSize++;
                    buffer.emplace_back((uint8_t) d.key.size());
                    bSize++;
                    for (auto c : d.key)
                    {
                        buffer.emplace_back((uint8_t) c);
                        bSize++;
                    }
                    ...
                    {
                        buffer.emplace_back((uint8_t) d.data.size());
                        bSize++;
                        for (auto p : d.data)
                        {
                            buffer.emplace_back(p);
